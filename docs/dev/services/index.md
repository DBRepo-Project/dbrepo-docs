...
the [Python Library](../../api/python) page for more details.
...
=== "UI"
    ...
    the system creates a user account in Figure 1 with the [default roles](auth-service#roles)
...
=== "UI"
    ...
    [Data Database](data-db) :material-numeric-2-circle-outline:.
...
=== "Terminal"
    ...
    The [Metadata Service](metadata-service) periodically (by default configuration every 60 seconds) checks
    and adds missing tables and views to the [Metadata Database](metadata-db), the database dump
    ...
    !!! example "Specifying a custom date format"
        ...
        administrator needs to add it manually in the [Metadata Database](metadata-db).
...
=== "JDBC"
    ...
    can persist the subset in the [Data Database](data-db) through JDBC by setting the `persisted`
...
=== "UI"
    ...
    First, provide information on the dataset creator(s). Since the [Metadata Service](metadata-service)
    ...
    Optionally add funding information, again the [Metadata Service](metadata-service)
    automatically resolves external PIDs, the easiest way is to provide the correct mandatory data is by filling the
...
=== "JDBC"
    ...
    !!! warning
        ...
        Creating a PID directly in the [Metadata Database](metadata-db) is not recommended! It bypasses
    ...
    Create a local PID directly in the [Metadata Database](metadata-db) by filling the tables in this
...
=== "JDBC"
    ...
    the [Metadata Database](metadata-db), change the visibility directly by executing the SQL-query
    in the `fda` schema:
