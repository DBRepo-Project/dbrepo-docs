...
By default, users are created using the [User Interface](ui) and the sign-up
page
...
the [Metadata Database](metadata-db) for faster access.
...
Users can register themselves through the [Auth service](auth-service) or via
